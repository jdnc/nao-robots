...
Vector2BH<> Geometry::rotate(const Vector2BH<>& v, float a)
{
  Vector2BH<> rV;
  float cos_a(cos(a));
  float sin_a(sin(a));
...
Vector2BH<int> Geometry::rotate(const Vector2BH<int>& v, float a)
{
  Vector2BH<int> rV;
  float cos_a(cos(a));
  float sin_a(sin(a));
  ...
  else
  {
    circle.radius =
      ...
      sqrt(
        ...
        /
        sqrBH(x2 * y1 - x3 * y1 - x1 * y2 + x3 * y2 + x1 * y3 - x2 * y3)
      );
  }
...
bool Geometry::calculatePointOnField(const Vector2BH<>& point, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector2BH<>& pointOnField)
{
...
bool Geometry::calculatePointOnField(const Vector2BH<>& image, const float& fieldZ, const CameraMatrix& cameraMatrix, const CameraInfo& cameraInfo, Vector3BH<>& field)
{
