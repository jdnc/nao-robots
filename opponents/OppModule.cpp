...
void OppModule::initSpecificModule(){
  ...
  oppLog((70, "done setting indiv opp model memory"));
...
void OppModule::reInit(){
  ...
  oppLog((70, "done setting opp model lua params"));
}
...
void OppModule::processFrame(){
  ...
  oppLog((30, "oppLoc processFrame start %5.3f, nModels: %i", frameInfo->seconds_since_start, getNumActiveModels()));
  ...
  if (previousGameState != gameState->state && previousGameState == INITIAL && !gameState->isPenaltyKick){
    ukfLog((10, "Changed state from initial, do player reset"));
    doOpponentReset();
  }
  // also if we changed teams
  else if (previousTeam != robotState->team_){
    ukfLog((10, "Team changed, do player reset"));
    doOpponentReset();
  }
  ...
  oppLog((30, "oppLoc processFrame complete %5.3f, nModels: %i", frameInfo->seconds_since_start, getNumActiveModels()));
}

void OppModule::doOpponentReset() {

  // Todd: by default, assume there is a keeper
  oppLog((30,"opp filter opponent reset - assume keeper"));
...
void OppModule::doPenaltyKickReset(){
  ...
  oppLog((30,"opp filter penalty kick reset"));
...
bool OppModule::clipModelToField(int modelID)
{
  ...
  if(clipped){
    oppLog((20, "Model %i, Alpha: %5.3f, Clipped 0, prev: (%5.1f, %5.1f), new: ((%5.1f, %5.1f) ", modelID, models[modelID].alpha, prevX, prevY, models[modelID].getState(0), models[modelID].getState(1)));
  }
  ...
  if(clipped){
    oppLog((20, "Model %i, Alpha: %5.3f, Clipped 1, prev: (%5.1f, %5.1f), new: ((%5.1f, %5.1f), ", modelID, models[modelID].alpha, prevX, prevY, models[modelID].getState(0), models[modelID].getState(1)));
  }
...
void OppModule::checkOpponentTeamPackets(){
  ...
  for (int i = WO_TEAM_FIRST; i <= WO_TEAM_LAST; i++){
    ...
    if (teamPacketsMem->oppUpdated[i]){
      ...
      if (teamPacketsMem->tp[i].bvrData.state != READY && teamPacketsMem->tp[i].bvrData.state != SET &&  teamPacketsMem->tp[i].bvrData.state != PLAYING){
        ukfLog((10, "ignore message from robot %i in state %i", i, teamPacketsMem->tp[i].bvrData.state));
        teamPacketsMem->oppUpdated[i] = false;
        continue;
      }

      //oppStruct* tp = &(teamPacketsMem->tp[i].oppData[i]);
      oppLog((20, "Do teammate opp update from mate %i recv frame %i", i, teamPacketsMem->frameReceived[i]));

      // loop through each  possible opponents in pkt
      for (int j = 0; j < MAX_OPP_MODELS_IN_MEM; j++){
        oppStruct* tp = &(teamPacketsMem->tp[i].oppData[j]);
        ...
        if (tp->framesMissed > 30){
          oppLog((20, "Teammate last saw opponent %i frames ago, no update", tp->framesMissed));
          continue;
        }

        if (isnan(tp->x) || isnan(tp->y) || tp->sdx == 0 || tp->sdxy == 0 || tp->sdy == 0 || (frameInfo->frame_id-teamPacketsMem->frameReceived[i]) > 15){
          oppLog((20, "Bad teammate opponent pkt, either nan, sd=0, or bad frame #"));
          teamPacketsMem->oppUpdated[i] = false;
          continue;
        }
...
int OppModule::doOpponentVisionUpdate(){
  ...
  oppLog((10, "update using self %i, loc (%5.3f, %5.3f), orient %5.3f, sd (%5.5f, %5.5f), sd orient %5.5f", robotState->WO_SELF, self->loc.x, self->loc.y, self->orientation, self->sd.x, self->sd.y, self->sdOrientation));
  ...
  for (int opp = WO_OPPONENT_FIRST; opp <= WO_OPPONENT_LAST; opp++) {
    ...
    oppLog((40, "Saw opponent at dist %5.3f, bearing %5.3f", oppDist, oppAng*RAD_T_DEG));

    // no models -> create one
    int numActiveModels = getNumActiveModels();
    if (numActiveModels == 0){
      oppLog((10, "No active opponent models, creating one"));
    ...
    for (int i = 0; i < c_MAX_MODELS; i++){
      if (models[i].isActive){
        oppLog((20, "attempt update on model %i", i));
        ...
        if (result == UKF4_OK){
          numSuccess++;
          oppLog((20, "successfully updated model %i with opp vision observation", i));
        }
      }
    }

    oppLog((10, "best model was %i with innov %5.3f", bestModel, bestInnov));

    // revert changes to all but the best model
    if (numSuccess > 1){
      for (int i = 0; i < c_MAX_MODELS; i++){
        if (models[i].isActive && i != bestModel){
          oppLog((20, "revert model %i", i));
          models[i].revert();
        }
      }
    }
    ...
    if (numSuccess == 0){
      oppLog((10, "All opponent models outliered on vision update, creating one"));
...
int OppModule::doOpponentBumpUpdate(float heading){
  ...
  oppLog((40, "Bump opponent at dist %5.3f, bearing %5.3f", dist, heading*RAD_T_DEG));
  ...
  for (int i = 0; i < c_MAX_MODELS; i++){
    if (models[i].isActive){
      ...
      if (result == UKF4_OK){
        numSuccess++;
        oppLog((20, "successfully updated model %i with opp bump observation", i));
      }
    }
  }
  ...
  // if they all outliered, create a new one
  if (numSuccess == 0){
    oppLog((10, "All opponent models outliered on bump update, creating one"));
    int modelID = FindNextFreeModel();
...
int OppModule::doOpponentSonarUpdate(){
  ...
  for (int opp = 0; opp < 3; opp++) {
    ...
    oppLog((40, "Sonar opponent at dist %5.3f, bearing %5.3f", oppDist, oppAng*RAD_T_DEG));
    ...
    for (int j = 0; j < c_MAX_MODELS; j++){
      if (models[j].isActive){
        ...
        if (result == UKF4_OK){
          numSuccess++;
          oppLog((20, "successfully updated model %i with sonar opp observation", j));
        }
      }
    }
    ...
    // if they all outliered, create a new one
    if (numSuccess == 0){
      oppLog((10, "All opponent models outliered on sonar, creating one"));
      int modelID = FindNextFreeModel();
      bool oppOk = initNewModel(modelID, oppDist, oppAng);
      if (oppOk){
...
bool OppModule::initNewModel(int modelID, float oppDist, float oppAng){
  ...
  if (fabs(oppX) > (GRASS_X/20.0) || fabs(oppY) > (GRASS_Y/20.0)){
    oppLog((10, "new opponent off grass, ignoring (%5.3f, %5.3f)", oppX, oppY));
...
int OppModule::doSharedOpponentUpdate(float x, float y, float sdxx, float sdxy, float sdyy){
  ...
  oppLog((20, "Shared Opponent Update. SharedOpp: (%5.1f, %5.1f) SRXX: %5.1f, SRXY %5.1f, SRYY %5.1f", x, y, sdxx, sdxy, sdyy));
  ...
  for (int i = 0; i < c_MAX_MODELS; i++){
    ...
    if (result == UKF4_OK){
      numSuccess++;
      oppLog((20, "successfully updated model %i with shared opp observation", i));
    }
  }
  ...
  // if they all outliered, create a new one
  if (numSuccess == 0){
    oppLog((10, "All opponent models outliered, creating one"));
    int modelID = FindNextFreeModel();
    models[modelID].init();
    models[modelID].isActive = true;
...
bool OppModule::MergeTwoModels(int index1, int index2)
{
  ...
  if(success == false){
    ...
    oppLog((10, "Merge models %i and %i FAILED", index1, index2));
...
{
  oppLog((20, "Model %i, Alpha: %5.3f, Active: %i, Activate: %i", modelID,models[modelID].alpha, models[modelID].isActive, models[modelID].toBeActivated));
  return;
}
  ...
  for (int i = 0; i < c_MAX_MODELS; i++) {
    for (int j = i; j < c_MAX_MODELS; j++) {
      ...
      if (mergeM < MergeMetricThreshold) { //0.5
        oppLog((40, "Merging models %i, alpha: %5.3f, into model %i, alpha: %5.3f, merge metric = %5.3f, thresh = %5.3f", j, models[j].alpha, i, models[i].alpha, mergeM, MergeMetricThreshold));
        MergeTwoModels(i,j);
      }
    }
  }
...
void OppModule::populateOpponents() {
  ...
  for (int j = 0; j < c_MAX_MODELS; j++){
    ...
    if (isnan(models[j].stateEstimates[0][0]) || isnan(models[j].sd(0))) continue;
    ...
    oppLog((40, "Fill in opponent %i from model %i: opponent at (%5.3f, %5.3f), sd (%5.3f, %5.3f)", oppID, j, opp->loc.x, opp->loc.y, opp->sd.x, opp->sd.y));
...
void OppModule::updateMemory() {
  ...
  for (i=0; i < MAX_OPP_MODELS_IN_MEM; i++) {
    opponentMem->alpha[i]=-1000; // set the models inactive by defualt
  }
  int tempCounter=0;
  for (i=0; i<c_MAX_MODELS; i++) {
    if (models[i].isActive) {
      opponentMem->modelNumber[tempCounter]=i;
      opponentMem->alpha[tempCounter]=models[i].alpha;
      opponentMem->X00[tempCounter]=models[i].stateEstimates[0][0];
      opponentMem->X10[tempCounter]=models[i].stateEstimates[1][0];
      opponentMem->X20[tempCounter]=models[i].stateEstimates[2][0];
      opponentMem->X30[tempCounter]=models[i].stateEstimates[3][0];

      oppLog((20, "Fill in opp memory %i from opp %i with sd: (%5.3f, %5.3f), last seen %i", tempCounter, i, models[i].sd(0), models[i].sd(1), models[i].frameUpdated));
      opponentMem->P00[tempCounter]=models[i].sd(0); //sd(0);
      opponentMem->P11[tempCounter]=models[i].sd(1); //sd(1);
      opponentMem->P01[tempCounter]=fabs(models[i].stateStandardDeviations[0][1]);
      opponentMem->P10[tempCounter]=opponentMem->P01[tempCounter];
      opponentMem->P22[tempCounter]=models[i].sd(2); //sd(2);
      opponentMem->P33[tempCounter]=models[i].sd(3);

      opponentMem->frameLastObserved[tempCounter] = models[i].frameUpdated;

      // sr for sharing
      NMatrix oppSR = models[i].GetLocSR();
      opponentMem->SRXX[tempCounter] = oppSR[0][0];
      opponentMem->SRXY[tempCounter] = oppSR[0][1];
      opponentMem->SRYY[tempCounter] = oppSR[1][1];
