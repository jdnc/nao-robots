...
void VisionModule::loadCalibration() {
  std::string calibrationFilename = boost::lexical_cast<std::string>(getRobotId()) + "_calibration.cal";
  std::string file = getDataBase() + calibrationFilename;
  bool loaded = false;
  RobotCalibration cal;
  if(!(loaded = cal.loadFromFile(file))) {
    file = getDataBase() + "default_calibration.cal";
    loaded = cal.loadFromFile(file);
  }
  if(loaded) {
    top_processor_->setCalibration(cal);
    top_processor_->enableCalibration(true);
    bottom_processor_->setCalibration(cal);
    bottom_processor_->enableCalibration(true);
    calibration_ = cal;
    printf("Loaded calibration: %s\n", file.c_str());
  }
  else {
    std::cout << "ERROR: No camera calibration found\n";
  }
}
