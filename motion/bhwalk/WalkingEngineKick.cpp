  ...
  while(*buf)
  {
    ...
    try
    {
      ...
      if(*buf != '#' && *buf != ';' && *buf != '\r' && *buf != '\n')
      {
        ...
        {
          preStepSizeXValue = readValueFloat(buf);
          preStepSizeYValue = readValueFloat(buf);
          preStepSizeZValue = readValueFloat(buf);
          readValue(buf);
          readValue(buf);
          preStepSizeRValue = readValueFloat(buf);
        }
        else if(str == "setStepSize")
        {
          stepSizeXValue = readValueFloat(buf);
          stepSizeYValue = readValueFloat(buf);
          stepSizeZValue = readValueFloat(buf);
          readValue(buf);
          readValue(buf);
          stepSizeRValue = readValueFloat(buf);
        }
...
bool WalkingEngineKick::load(const char* filePath)
{
  ...
  if(success)
  {
    ...
    std::cout << "ERROR LOADING KICK: " << filePath << std::endl;
  }
  return success;
}
...
{
  rotation = preStepSizeRValue;
  translation.x = preStepSizeXValue;
  translation.y = preStepSizeYValue;
  translation.z = preStepSizeZValue;
}

void WalkingEngineKick::getStepSize(float& rotation, Vector3BH<>& translation) const
{
  rotation = stepSizeRValue;
  translation.x = stepSizeXValue;
  translation.y = stepSizeYValue;
  translation.z = stepSizeZValue;
}
...
void WalkingEngineKick::setParameters(const Vector2BH<>& ballPosition, const Vector2BH<>& target)
{
